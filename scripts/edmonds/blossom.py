...
from edmonds.graph_utils import *
...
def matching_edmonds(G: nx.Graph, pos: dict, show: bool = False,
                     options: dict = {"font_color" : "orangered", "node_color" : "white",
                                       "edgecolors"  : "darkorange", "linewidths" : 3},
                     figsize: Tuple[int,int] = (30,30)) -> Tuple[nx.Graph, List[str], List[int], List[Tuple[str,str]]]:
    """Find the maximum cardinality matching in the graph G and returns the graph with selected edges"""
    ...
    # If a non-empty matching is found by the algorithm, return the matching and the graph with selected edges
    if matching != []:
